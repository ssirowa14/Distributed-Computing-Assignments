...
public:
    ...
    atomic<bool> tokenHas;																  //if you have the token
	atomic <bool> enumBit;                                                                //enum Bit
	token tokenVal;
	int parent = -1;
	int port;
    ...
    int send_(msg m1, int recv, vector <Node> &nodeList){
		...
		if ((client_sock = socket(AF_INET, SOCK_STREAM, 0)) < 0){								//error handling
    	...
    	while(!term_detect)														         //loop
    	{
			...
			if(tokenHas == true){
				...
				if(id == N-1 && localIdle == true){										//coordinator
					tokenHas = false;
					if(localColor == 0 &&(tokenVal.counterSum+localCounter) == 0 && tokenVal.color == 0){ //alglo logic
						term_detect = true;
			...
			else
			{
				...
				if(reply.msgType == 0){
					...
					if(reply.sender_state == 1)                                                         //if the process who sent msg was red/active 
					{
						...
						mtx.lock();
						idle = false; 
						                                                                            //now this process can become active
						if(reply.enumBit != enumBit && id > reply.sender_id)
						{
